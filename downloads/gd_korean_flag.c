  ...
  void draw_korean_flag(gdImagePtr img) {
      ...
      // gdImageRectangle(img, 0, 0, width, height, black); // Commented out to remove border
      ...
      gdImageFilledPolygon(img, points, 4, grey);
      // rectangle rotate to lower ritht// https://en.wikipedia.org/wiki/Flag_of_South_Korea
    // https://en.wikipedia.org/wiki/Flag_of_South_Korea#/media/File:Flag_of_South_Korea_(construction_sheet).svg
    #include <stdio.h>
    #include <gd.h>
    #include <math.h>
    ...
    void draw_korean_flag(gdImagePtr img) {
      ...
      // Draw the big circle outline
      int circle_radius = (int)(height * 2.0 / 5.0);
      // gdImageEllipse(img, center_x, center_y, big_circle_dia, big_circle_dia, black); // Commented out to remove border
